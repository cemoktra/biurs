...
async fn main() -> anyhow::Result<()> {
    ...
    let client = biurs_core::client::BiursClient::new(server).await?;
                    ...
                    match entry {
                        Ok(entry) => {
                            if entry.file_type().is_file() {
                                ...
                                let status = client
                                    .status(biurs_core::types::status::StatusRequest {
                                        file: entry.clone(),
                                ...
                                match status {
                                    biurs_core::types::status::StatusResponse::Missing => {
                                        ...
                                        client
                                            .upload(biurs_core::types::upload::UploadFile {
        ...
        SubCommand::Restore => {
            ...
            for meta in metas {
                ...
                let downloaded = client
                    .download(biurs_core::types::download::DownloadRequest {
